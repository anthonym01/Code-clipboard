...
const { dialog, Menu, MenuItem } = require('electron').remote;//Initalize dialogue dependency
...
function body_menu() {
    const Body_menu = new Menu.buildFromTemplate([
        {
            label: 'Hide pannel', click() {
                utility.minimize()
            }
        },
        { role: 'toggleDevTools' }
    ...
    window.addEventListener('contextmenu', function () {
        ...
        Body_menu.popup({ window: require('electron').remote.getCurrentWindow() })
    })
}
...
//document.getElementById('close_btn').addEventListener('click', utility.close)
document.getElementById('minimize_btn').addEventListener('click', utility.minimize)
